...
// You can add this function to a new or existing model, for example, your Attendance Model.

    public function getAbsenceRemarks(int $emp_id, string $date): string
    {
        // Check if the day is a holiday
        if ($this->holidayModel->checkIFDayIsHoliday($date)) {
            return 'holiday';
        }
        ...
        // Check for an F-day
        if ($this->fdayModel->checkIFDayIsFday($emp_id, $date)) {
            return 'fday';
        }

        // Check for medical leave
        if ($this->medModel->checkIFDayIsMed($emp_id, $date)) {
            return 'medical';
        }

        // Check for regular leave
        if ($this->leaveModel->isLeaveCoveringDate($emp_id, $date)) {
            return 'onleave';
        }
        
        // If none of the above, it's an unexcused absence
        return 'absent with no excuse';
    }
    ...
    public function getFingerPrintForEmpPeriod($filters): array
    {
        ...
        for ($current_month_ts = $start_period_ts; $current_month_ts <= $end_period_ts; $current_month_ts = strtotime('+1 month', $current_month_ts)) {
            ...
            foreach ($daily_records_raw as $fp_record) {
                ...
                if (isset($processed_fp_record->absent) && $processed_fp_record->absent === 'TRUE') {
                    $processed_fp_record->remarks = $this->getAbsenceRemarks($emp_id, $processed_fp_record->date);
                } else {
                    $processed_fp_record->remarks = '';
                }
    ...
    public function getFingerPrint($filters): array
    {
        ...
        foreach ($emps as $emp) {
            if ($emp->join_date <= $to_date && (is_null($emp->termination_date) || $emp->termination_date >= $from_date)) {
    ...
    private function getEmptyAttendData(int $emp_id, string $month, int $year): array
    {
        return [
            ...
            'absent_no_excusie_days_list' => [], 'absent_no_excusie_days' => 0,
    ...
    public function getAttendForEmp($filters, $preloaded_data): array
    {
        ...
        while ($current_day_ts <= strtotime($to)) {
            ...
            if (!$attend_data_for_day || (isset($attend_data_for_day->absent) && $attend_data_for_day->absent === 'TRUE')) {
                ...
                if (!$isFriday && !$isHoliday && !$isFday) {
                    if ($isMed) {
                        $meds_days_without_fridays_list[] = $date_str;
                        $has_med = true;
                    }else{
                        if(!$isLeave){
                            $absent_no_excusie_days_list[] = $date_str;
                        }
                    }
                }
            } else {
        ...
        $final_calculated_working_days = ($this->month_working_days - count($absent_no_excusie_days_list) - count($leaves_days_without_fridays_list) - count($meds_days_without_fridays_list));
        ...
        return [
            ...
            'absent_no_excusie_days_list' => array_unique($absent_no_excusie_days_list),
            'absent_no_excusie_days' => count(array_unique($absent_no_excusie_days_list)),
