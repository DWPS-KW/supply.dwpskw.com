...
<?= $this->section('title'); ?><?= $pageTitle ?? 'احصائيات المرضيات'; ?><?= $this->endSection(); ?>
...
<div class="container-fluid pt-6">
    ...
    <form action="<?= base_url('medicals'); ?>" method="get">
                    ...
                    <select class="form-control" id="sec_id" name="sec_id">
                        ...
                        <?php if (session('type') == "admin"): ?>
                            <?php foreach($secs as $sec): ?>
                                <option value="<?= $sec->id; ?>" <?= ($sec->id == ($filters['sec_id'] ?? null)) ? 'selected' : ''; ?>>
                                    <?= $sec->name_arabic; ?>
                                </option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                ...
                <div class="form-group text-right">
                    ...
                    <select class="form-control" id="sub_sec_id" name="sub_sec_id">
                        ...
                        <?php if (session('type') == "depart"): ?>
                            <?php if (!empty($sub_secs)): ?>
                                <?php foreach ($sub_secs as $sub_sec): ?>
                                    <option value="<?= $sub_sec->id; ?>" <?= ($sub_sec->id == ($filters['sub_sec_id'] ?? null)) ? 'selected' : ''; ?>>
                                        <?= $sub_sec->name_arabic; ?>
                                    </option>
                                <?php endforeach; ?>
                            <?php endif; ?>
            ...
            <div class="col-sm-6 col-md-4 col-lg-3">
                <div class="form-group text-right">
                    ...
                    <select class="form-control" id="arrange_by" name="arrange_by">
                        ...
                        <option value="date" <?= isset($arrange_by) && $arrange_by === 'date' ? 'selected' : ''; ?>>التاريخ
                        </option>
                    </select>
                </div>
            </div>
        ...
        <div class="row" style="text-align: left;">
            <div class="col-sm-6 col-md-4 col-lg-3">
                <div class="form-group text-right">
                    ...
                    <?php $query = http_build_query($filters ?? []); ?>

                    <a href="<?= base_url('printing/medicals?' . $query); ?>" target="_blank">
                    ...
                    &nbsp; &nbsp; &nbsp;
                        ...
                        <a href="<?= base_url('medicals/exportToExcel?' . $query); ?>" class="btn btn-success">
                            <i class="fa fa-file-excel-o"></i> تحميل Excel
                        </a>

                    &nbsp; &nbsp; &nbsp;

                    <a href="<?= base_url('medicals'); ?>">
                        <button type="button" class="btn fav">
                            إلغاء البحث
                        </button>
                    </a>
    ...
    <div class="row">
        ...
        <div class="col-12" id="srchRes_mobile">
            ...
            <?php foreach ($result['pagedMeds'] as $row) { ?>
        ...
        <table class="table table-striped table-hover display" id="srchRes_desktop" style="direction: rtl;">
            ...
            foreach ($result['pagedMeds'] as $row) { // Corrected key to 'pagedMeds'
                ?>
    ...
    <?php if ($totalPages > 1): ?>
        <ul class="pagination">
            <li class="<?= ($currentPage == 1) ? 'disabled' : ''; ?>">
                <a href="<?= base_url('medicals?' . $queryString . '&page=' . ($currentPage - 1)); ?>" aria-label="Previous">
                    <span aria-hidden="true">&laquo;</span>
                </a>
            </li>
            <?php for ($p = 1; $p <= $totalPages; $p++): ?>
                <li class="<?= ($p == $currentPage) ? 'active' : ''; ?>">
                    <a href="<?= base_url('medicals?' . $queryString . '&page=' . $p); ?>"><?= $p; ?></a>
                </li>
            <?php endfor; ?>
            <li class="<?= ($currentPage == $totalPages) ? 'disabled' : ''; ?>">
                <a href="<?= base_url('medicals?' . $queryString . '&page=' . ($currentPage + 1)); ?>" aria-label="Next">
...
<script>
    $(document).ready(function() {
        ...
        <?php if (session('type') == "admin" || session('type') == "depart") : ?>
            ...
            var selected_sub_sec_id = "<?= $filters['sub_sec_id'] ?? '' ?>";
